...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="flex flex-col gap-5 md:gap-7 2xl:gap-10">
            ...
            <div class="data-table-common data-table-one max-w-full overflow-x-auto">
                ...
                <table class="table w-full table-auto" id="dataTableOne">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <a href="{{route('show_student_apply', $item->id)}}">
                            ...
                            <td><a class="text-black" href="{{route('show_student_apply', $item->id)}}">
                                    {{$item->institute_name}}
                                </a></td>
                            ...
                            <td>
                                ...
                                <p
                                    class="ursor-pointer text-center rounded-full bg-warning bg-opacity-10 px-3 py-1 text-sm font-medium text-warning">
                                    Pending
                                </p>
                                @elseif($item->payment_status == 'reject')
                                <p
                                    class="ursor-pointer text-center rounded-full bg-danger bg-opacity-10 px-3 py-1 text-sm font-medium text-danger">
                                    Reject
                                </p>

                                @elseif($item->payment_status == 'paid')
                                <p
                                    class="ursor-pointer text-center rounded-full bg-success bg-opacity-10 px-3 py-1 text-sm font-medium text-success">
                                    Paid
                                </p>
                                @endif
                                ...
                                <form id="myForm" class="text-center"
                                    action="{{ route('updatePaymentStatus', $item->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                            ...
                            <td>
                                ...
                                <form class="text-center" action="{{ route('updateStatus', $item->id) }}" method="POST">
                                    ...
                                    <select name="status" id="status" onchange="this.form.submit()"
                                        ...
                                        <option
                                            class="text-center rounded-full bg-warning bg-opacity-10 px-3 py-1 text-sm font-medium text-warning"
                                            value="pending" {{ $item->status == 'pending' ? 'selected' : '' }}>Pending
                                        </option>
                                        <option
                                            class="text-center rounded-full bg-danger bg-opacity-10 px-3 py-1 text-sm font-medium text-danger"
                                            value="reject" {{ $item->status == 'reject' ? 'selected' : '' }}>Reject
                                        </option>
                                        <option
                                            class="text-center rounded-full bg-success bg-opacity-10 px-3 py-1 text-sm font-medium text-success"
                                            value="accept" {{ $item->status == 'accept' ? 'selected' : '' }}>Accept
                                        </option>
                                    </select>
                                </form>
                            </td>
        ...
        <!-- ====== Data Table One End -->
        <script>
            document.getElementById('downloadCollege').addEventListener('click', function() {
            let csv = 'Student ID,Institute Name,Education,Payment Status, Status\n';
            document.querySelectorAll('#dataTableOne tbody tr').forEach(row => {
                const cells = row.querySelectorAll('td');
                csv +=
                    `${cells[0].innerText},${cells[1].innerText},${cells[2].innerText},${cells[3].innerText},${cells[4].innerText},${cells[5].innerText}\n`;
            });
